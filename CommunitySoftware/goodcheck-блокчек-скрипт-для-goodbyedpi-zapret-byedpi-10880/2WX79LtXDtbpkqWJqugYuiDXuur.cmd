...
set "dohEnabled=true"
set "curlDoH=https://one.one.one.one/dns-query"
set "customCommonResolverEnabled=false"
set "customResolverIPv=4"
set "resolver=1.1.1.1"
...
call :OverrideStringParam "!_curlDoH!" curlDoH
call :OverrideBooleanParam "!_customCommonResolverEnabled!" customCommonResolverEnabled
call :OverrideStringParam "!_resolver!" resolver
...
if "!dohEnabled!"=="true" (
	call :WriteToConsoleAndToLog Setting up DNS-over-HTTPS resolver... ^(!curlDoH!^)
...
if "!dohEnabled!"=="false" if "!customCommonResolverEnabled!"=="true" (
	...
	call :WriteToConsoleAndToLog Setting up DNS resolver... ^(!resolver!^)
)
...
for /L %%i in (0,1,10) do (
	if not defined result (
		if not "!reportMappingURL[%%i]!"=="" (
			for /F "usebackq tokens=3 delims= " %%j in (`"!curl!" !curlExtraKeys! -sm !curlMinTimeout! !reportMappingURL[%%i]!`) do (
				set "result=%%j"
			)
		)
	)
