...
set "mappingURL=https://redirector.gvt1.com/report_mapping?di=no"
::set "mappingURL=https://redirector.googlevideo.com/report_mapping?di=no"
set customcluster=%*
set cluster=0
set "DefaultText=[0m"
set "RedText=[91m"
...
echo Press %GreenText%Enter%DefaultText% to automatically test closest cluster of Google Cache Servers
echo or %GreenText%input googlevideo URL%DefaultText% to test corresponding cluster
echo.
echo examples: https://rr2---sn-4axm-n8vs.googlevideo.com
echo           rr2---sn-gvnuxaxjvh-o8ge.googlevideo.com
echo           https://rr10---sn-axq7sn7z.googlevideo.com/videoplayback?expire=1727433844...
echo.
set /P customcluster="Input: "
echo.
if defined customcluster (goto PARSECUSTOMCLUSTER)

::Auto cluster
for /F "tokens=3 delims= " %%i in ('curl%proxy% -sm %LaxTimeout% %mappingURL%') do (set cluster=%%i)
if %cluster%==0 ((echo %RedText%Failure! Cannot establish connection to %mappingURL% to find a cluster codename%DefaultText%) && (goto EOF))
