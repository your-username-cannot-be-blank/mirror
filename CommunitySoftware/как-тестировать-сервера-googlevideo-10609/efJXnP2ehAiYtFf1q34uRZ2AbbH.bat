...
chcp 1251
@echo off
title Testing Closest Google Cache Servers
cls
...
::Claster codename search
echo.
echo %InverseText%Looking for claster codename%DefaultText%
echo.
if not defined customclaster (goto USERINPUTCLASTER)
:PARSECUSTOMCLASTER
::Parsing custom claster codename
for /F "tokens=1 delims=." %%i in ("%customclaster%") do (set "customclaster=%%i")
for /F "tokens=1* delims=-" %%i in ("%customclaster%") do (set "customclaster=%%j")
set "claster=%customclaster%"
echo %BoldText%Claster codename:%DefaultText% %claster%
set "claster=---%claster%"
goto PINGING

:USERINPUTCLASTER
::Custom input by user
set /P customclaster="Input googlevideo URL or press ENTER for auto: "
echo.
if not defined customclaster (goto SEARCHCLASTER) else (goto PARSECUSTOMCLASTER)

:SEARCHCLASTER
::Searching claster codename
for /F "tokens=3 delims= " %%i in ('curl -sm 2 "https://redirector.googlevideo.com/report_mapping?di=no"') do (set claster=%%i)
if %claster%==0 ((echo %RedText%Failure! Cannot establish connection to googlevideo.com to find claster codename%DefaultText%) && (goto EOF))
echo %BoldText%Claster codename:%DefaultText% %claster%
set "claster=.%claster%"
...
set ggcCounter=1
:URLLISTLOOP
set ggcURL=rr%ggcCounter%%claster%.googlevideo.com
echo %BoldText%Pinging%DefaultText% %ggcURL%...
ping -n 1 %ggcURL% >NUL
if %ERRORLEVEL%==0 ((echo %GreenText%Success%DefaultText%) && (set /A ggcCounter=%ggcCounter%+1) && (echo.) && (goto URLLISTLOOP))
...
set Counter=1
:TCPCONNLOOP
set "ggcURL=https://rr%counter%%claster%.googlevideo.com"
