...
curl%proxy% -sko NUL -m %Timeout% -w "DNS_T: %%{time_namelookup} µs | Connection_T: %%{time_connect} µs | Handshake_T: %%{time_appconnect} µs | Total_T: %%{time_total} µs | Header: %%{size_header} bytes\n" %ggcURL%
...
set "ggcURL=https://rr%counter%%cluster%.googlevideo.com"
echo %BoldText%Testing%DefaultText% %ggcURL%...
curl%proxy% -sko NUL -m %Timeout% --http3-only -w "DNS_T: %%{time_namelookup} µs | Connection_T: %%{time_connect} µs | Handshake_T: %%{time_appconnect} µs | Total_T: %%{time_total} µs | Header: %%{size_header} bytes\n" %ggcURL%
...
curl%proxy% -sko NUL -m %Timeout% %ggcURL%
if !ERRORLEVEL!==0 (set /A successes+=1)
)
...
SetLocal EnableDelayedExpansion
for /L %%i in (1,1,%attempts%) do (
curl%proxy% -sko NUL --http3-only -m %Timeout% %ggcURL%
if !ERRORLEVEL!==0 (set /A successes+=1)
if !ERRORLEVEL!==95 (set /A successes+=1)
)
if !successes!==%attempts% (set clr=%GreenText%) else (set clr=%RedText%)
echo %BoldText%Succesful connections:%DefaultText% !clr!!successes! out of %attempts%%DefaultText%
echo.
