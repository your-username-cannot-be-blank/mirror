...
curl_with_subst_ip()
{
	...
	ALL_PROXY="$ALL_PROXY" 
	$CURL ${arg:+$arg }$connect_to "$@" 2>/dev/null
	code=$?
	#if [ $code = 0 ]; then
		#echo "!!!!! AVAILABLE !!!!! $CURL ${arg:+$arg }$connect_to "$@""
	#fi
	return $code
}
curl_with_dig()
...
pktws_start()
{
	case "$UNAME" in
		...
		FreeBSD|OpenBSD)
			...
			;;
		CYGWIN)
			#echo "f=pktws_start" STR="$WINWS" $WF "$@"
			"$WINWS" $WF "$@" >/dev/null &
			;;
	esac
	PID=$!
...
curl_test()
{
	...
	#[ "$4" = detail ] || {
		#if [ $code = 0 ]; then
			#echo "f=curl_test !!!!!AVAILABLE code=$code"
		#else
			#echo "f=curl_test !!!!!UNAVAILABLE code=$code"
		#fi
	#}
	return $code
}
...
pktws_curl_test()
{
	...
	code=$?
	if [ $code = 0 ]; then
		echo "!!!!! AVAILABLE !!!!!" $PKTWSD ${WF:+$WF }$3 $4 $5 $6 $7 $8 $9 $PKTWS_EXTRA
	fi
}
