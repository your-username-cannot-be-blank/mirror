...
function installQuestions() {
	...
	read -rp "Public IPv4 address or domain: " -e -i "${SERVER_PUB_IP}" SERVER_PUB_IP
	...
	until [[ ${SERVER_AWG_IPV4} =~ ^([0-9]{1,3}\.){3} ]]; do
		read -rp "Server AmneziaWG IPv4: " -e -i 10.29.8.1 SERVER_AWG_IPV4
	done
	...
	until [[ ${CLIENT_DNS_1} =~ ^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$ ]]; do
		read -rp "First DNS resolver to use for the clients: " -e -i 10.29.0.1 CLIENT_DNS_1
	done
	until [[ ${CLIENT_DNS_2} =~ ^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$ ]]; do
		read -rp "Second DNS resolver to use for the clients (optional): " -e -i 10.29.8.1 CLIENT_DNS_2
	...
	until [[ ${ALLOWED_IPS} =~ ^.+$ ]]; do
		...
		read -rp "Allowed IPs list for generated clients (leave default to route everything): " -e -i '10.29.0.0/16,10.30.0.0/15' ALLOWED_IPS
		if [[ ${ALLOWED_IPS} == "" ]]; then
			ALLOWED_IPS="10.29.0.0/16,10.30.0.0/15"
		fi
	done
